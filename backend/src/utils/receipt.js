...
const generateRegistrationPDF = async ({ payment, registration, ticket }) => {
  if (!payment) {
    throw new Error("Payment is required to generate registration PDF.");
  }

  if (!registration) {
    throw new Error("Registration is required to generate registration PDF.");
  }

  if (!ticket) {
    throw new Error("Ticket is required to generate registration PDF.");
  }
  ...
  const qrPayload = qrUtil.generateTicketQRPayload({
    ticketNumber: ticket.ticketNumber,

    qrToken: ticket.qrToken,
  });

  const qrBuffer = await qrUtil.generateQRBuffer(qrPayload, {
    width: 500,
    margin: 2,
  });
  ...
  const event = registration.event || ticket.event || {};

  const festival = registration.festival || ticket.festival || {};
  ...
  if (ticket.teamMemberId && registration.team && registration.team.members) {
    ...
    participantName =
      registration.participantName || user.fullName || "Participant";
  ...
  return new Promise((resolve, reject) => {
    ...
    const document = new PDFDocument({
      ...
      info: {
        Title: "SCINTILLACE Event Registration Confirmation",
    ...
    document.fontSize(26).font("Helvetica-Bold").text("SCINTILLACE", {
      align: "center",
    });
    ...
    document.moveDown(1).fillColor("#000000");
    ...
    document.fontSize(15).font("Helvetica-Bold").text("Registration Confirmed");

    document
      .moveDown(0.5)
      .fontSize(10.5)
      .font("Helvetica")
      .text(`Dear ${safeText(participantName, "Participant")},`);
    ...
    const participantRows = [
      ...
      ["Phone", safeText(participantPhone)],

      ["College ID", safeText(collegeId)],
    ];

    if (department) {
      participantRows.push(["Department", safeText(department)]);
    }

    if (yearOfStudy) {
      participantRows.push(["Year of Study", safeText(yearOfStudy)]);
    }

    if (teamName) {
      participantRows.push(["Team Name", safeText(teamName)]);
    }

    if (teamRole) {
      participantRows.push(["Team Role", safeText(teamRole)]);
    }

    if (projectTitle) {
      participantRows.push(["Project Title", safeText(projectTitle)]);
    }

    participantRows.forEach(([label, value]) => {
      document
        .fontSize(10)
        .font("Helvetica-Bold")
        .text(`${label}: `, {
    ...
    const eventDate = new Date(event.startDateTime).toLocaleString("en-IN", {
      dateStyle: "long",
    });

    const startTime = new Date(event.startDateTime).toLocaleString("en-IN", {
      timeStyle: "short",
    });

    const endTime = new Date(event.endDateTime).toLocaleString("en-IN", {
      timeStyle: "short",
    });

    const eventRows = [
      ["Event", safeText(event.title)],

      ["Category", safeText(event.category)],

      ["Date", safeText(eventDate)],
    ...
    const ticketRows = [
      ["Registration ID", safeText(registrationId)],

      ["Ticket Number", safeText(ticket.ticketNumber)],

      ["Ticket Status", safeText(ticket.status)],
    ...
    const paymentRows = [
      ...
      ["Verified At", formatDate(payment.paidAt || new Date())],
    ];
    ...
    const qrX = (document.page.width - qrSize) / 2;
...
const generateAccommodationReceiptPDF = async ({ accommodation }) => {
  if (!accommodation) {
    throw new Error(
  ...
  if (accommodation.paymentStatus !== "Paid") {
    throw new Error(
      "Accommodation payment must be completed before generating the receipt.",
    );
  }

  const user = accommodation.user || {};

  const registration = accommodation.registration || {};

  const event = accommodation.event || registration.event || {};

  const festival = event.festival || registration.festival || {};
  ...
  if (accommodation.confirmationCode) {
    const qrPayload = `SCINTILLACE-ACCOMMODATION:${accommodation.confirmationCode}`;

    qrBuffer = await qrUtil.generateQRBuffer(qrPayload, {
      width: 350,
      margin: 2,
    });
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const document = new PDFDocument({
      ...
      info: {
        Title: "SCINTILLACE Accommodation Payment Receipt",
    ...
    document
      ...
      .font("Helvetica")
      .fillColor("#666666")
      .text("Accommodation Payment Receipt", {
        align: "center",
      });
    ...
    document.fontSize(16).font("Helvetica-Bold").text("Payment Successful", {
      align: "center",
    });
    ...
    document
      ...
      .fontSize(10.5)
      .font("Helvetica")
      .text(`Dear ${safeText(user.fullName, "Participant")},`);
    ...
    const participantRows = [
      ...
      ["College ID", safeText(user.collegeId)],
    ];
    ...
    const registrationRows = [
      ["Festival", safeText(festival.title, "SCINTILLACE 2K26")],

      ["Event", safeText(event.title)],

      ["Registration ID", safeText(registration._id || registration.id)],
    ];

    registrationRows.forEach(([label, value]) => {
      document
        .fontSize(10)
        .font("Helvetica-Bold")
        .text(`${label}: `, {
    ...
    const accommodationRows = [
      [
        "Hostel",
        accommodation.hostelType === "BOYS"
          ? "Boys Hostel"
          : accommodation.hostelType === "GIRLS"
            ? "Girls Hostel"
            : safeText(accommodation.hostelType),
      ],
      ...
      ["Accommodation Days", safeText(accommodation.accommodationDays, "0")],

      ["Rate", `${formatAmount(100, "INR")} per day`],
    ];

    accommodationRows.forEach(([label, value]) => {
      document
        .fontSize(10)
        .font("Helvetica-Bold")
        .text(`${label}: `, {
    ...
    const paymentRows = [
      [
        "Amount Paid",
        formatAmount(accommodation.amount, accommodation.currency),
      ],
      ...
      [
        "Verified At",
        formatDate(
          accommodation.paidAt || accommodation.payment?.paidAt || new Date(),
        ),
      ],
    ];
    ...
    if (qrBuffer) {
      document
        ...
        .fontSize(12)
        .font("Helvetica-Bold")
        .text("Accommodation Verification QR", {
          align: "center",
        });
    ...
    document.moveDown(0.3).text("SCINTILLACE Team", {
      align: "center",
    });
...
const receiptUtil = Object.freeze({
  generateReceiptPDF,
